...
public class TestPacManValidMoves extends TestCase {
  ...
  public void testPacManValidMoves() throws FileNotFoundException {
    ...
    // Right column and bottom row are filled with cookies
    ...
    // Pacman is not able to move up or to the left
    validMoves = testPacMan.get_valid_moves();
    validMoves.forEach((location) -> assertTrue(location.x != 0 && location.y != 0));
    assertTrue(validMoves.size() == 3);

    // Test for no moves if pacman is surrounded
    testMap = new Map(3);
    for (int row = 0; row < 3; row++) {
      for (int col = 0; col < 3; col++) {
        Location loc = new Location(row, col);
        if (loc.x == loc.y && loc.x == 1) {
          // Place pacman at the center of the map
          testPacMan = new PacMan("test", loc, testMap);
        } else {
